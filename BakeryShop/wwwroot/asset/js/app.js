...
const mssgList = document.querySelector(`.mssg-list`);
const sub = document.querySelector(`.sub-menu-admin-container`);
const sub_list_mssg = document.querySelector(`.message-sub-list`);
adminMenu.addEventListener(`click`, function (event) {
  sub.classList.toggle(`hidden`);
  event.stopPropagation();
});
mssgList.addEventListener(`click`, function (event) {
  sub_list_mssg.classList.toggle(`hidden`);
  event.stopPropagation();
});
